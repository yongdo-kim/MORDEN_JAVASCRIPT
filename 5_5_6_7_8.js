console.log("-------  iterable  START --------");
/*
    iterable 객체 : 반복 가능한(iterable, 이터러블) 객체
    Symbol.iterator을 이용해서 이터러블 객체를 만들 수 있다.

    목표 : 
        - let ragne {from :1, to :5} 객체를 반복문으로 돌려보자.
        - 작동순서 : 
            for (let num of range)를 호출    
            loop문이기 때문에 [Symbol.iterator]를 호출   
            내부 객체에 Symbol.iterator가 없다면 호출
            다음 값이 필요하면 next 함수 호출(next는 예약어)
...
console.log("-------  Weak Map, Weak Set START --------");
/*
    위크맵과 위크셋
    위크맵
...
console.log("-------  Weak Map, Weak Set END --------");